...
The _Copy selector_ functionality allows for fast access to the CSS selector that can be used to select a Markdown element in the Markdown source text. When used on an element, it will _copy_ the selector to the pastboard which can then be  pasted in the stylesheet source using the menu or the keyboard shortcut.  
