...
final class CSSDOMColorValueElement: CSSDOMElement, CSSDOMSelectableValueElement {
    ...
    public override func equals(to other: Any?, comparePositions: Bool = false) -> Bool {
        ...
        if let other = other {
            ...
            if let other = other as? CSSDOMColorValueElement {
                ...
                #if (os(macOS) || os(iOS) || os(tvOS) || os(watchOS)) && DEBUG && DEBUG_LOGS_ENABLED
                os_log("Not equals: other is not CSSDOMColorValueElement.", log: Log.Web.all, type: .debug)
                #endif
                return false
            }
        }
