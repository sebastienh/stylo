//
//  CSSStyleSheetSet+SequenceType.swift
//  Web
//
...
extension CSSStyleSheetSet : Sequence {
    
    // SequenceType protocol implementation
    func makeIterator() -> CSSStyleSheetSetGenerator {
        
        return CSSStyleSheetSetGenerator(styleSheetList: self.styleSheetList)
    }
}

// see http://lillylabs.no/2014/09/30/make-iterable-swift-collection-type-sequencetype/
final class CSSStyleSheetSetGenerator : IteratorProtocol {
    
    // Hold the collection to be iterated through
    fileprivate var styleSheetList: StyleSheetList
    ...
    // to the car array
    init(styleSheetList: StyleSheetList) {
    ...
    func next() -> CSSStyleSheet? {
        ...
        if (nextIndex >= styleSheetList.length) {
            return nil
        }
