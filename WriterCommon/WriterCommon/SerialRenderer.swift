...
protocol SerialRenderer: Renderer {
    ...
    func text(from element: Element) -> String?
    ...
    func paintTextElement(element: Element, updateType: PaintingUpdateType)
    
    func paintTextElement(element: Element, updateType: PaintingUpdateType, exclusionRanges: inout [NSRange])
    
    func paintPseudoElements(element: Element, updateType: PaintingUpdateType, exclusionRanges: inout [NSRange])
    
    func erasePseudoElements(element: Element, exclusionRanges: inout [NSRange])
    
    func paragraphApply(attributes: [NSAttributedString.Key : AnyObject]?, to range: NSRange)
    
    func update(attributes: [NSAttributedString.Key : Any]?, in range: NSRange, from element: Element, updateType: PaintingUpdateType, removeBackgroundColor: Bool)
    
    func remove(attributes: [NSAttributedString.Key : Any]?, in range: NSRange, from element: Element)
    
    func eraseNodes(nodes: ContiguousArray<Node>)
...
extension SerialRenderer {
    ...
    func text(from element: Element) -> String? {
        ...
        let ranges = element.sourceStringFragment?.ranges
        if let ranges = ranges {
            var text = ""
            for range in ranges {
                if let rangeString = contentString.substring(from: range) {
                    text += rangeString
                }
            }
            return text
...
//                    assertionFailure("Error: style was nil")
    ...
    ///
    func paintTextElement(element: Element, updateType: PaintingUpdateType = .add) {
...
//                element.logRendering(filterContext: self.filterContext)
        ...
        self.resourceComputedStyle.evaluateEphemeralStyle(for: element, filterContext: self.filterContext)
        paintTextElement(element: element, updateType: updateType, exclusionRanges: &exclusionRanges)
    ...
    func paintTextElement(element: Element, updateType: PaintingUpdateType = .add, exclusionRanges: inout [NSRange]) {
        ...
        if let computedStyle = self.style(for: element) {
            ...
            if let attributes = attributes, attributes.count > 0 {
                ...
                #if (os(macOS) || os(iOS) || os(tvOS) || os(watchOS)) && DEBUG && DEBUG_LOGS_ENABLED
                if let text = self.text(from: element) {
                    os_log("Paint element: %@ with class: %@ and text: %@", log: Log.WriterCommon.all, type: .info, %%element.localName, %%element.classListString, %%text)
                }
                #endif
                ...
                /// paint themselves.
                paintPseudoElements(element: element, updateType: updateType, exclusionRanges: &exclusionRanges)
                            ...
                            if let sourceStringFragment = textChild.sourceStringFragment {
                                ...
                                if let ephemeralRanges = self.filterContext.ephemeralRanges(forElement: element, fromRanges: ranges) {
                                    ...
                                    for range in ephemeralRanges {
                                        paintRange(range: range, element: element, textAttributes: textAttributes, updateType: updateType, exclusionRanges: &exclusionRanges)
                                    }
                                }
                            }
                        }
    ...
    private func paintRange(range: NSRange, element: Element, textAttributes: [NSAttributedString.Key : Any], updateType: PaintingUpdateType = .add, exclusionRanges: inout [NSRange]) {
        ...
        if !exclusionRanges.isEmpty {
            ...
            for elementSpecificRange in elementSpecificRanges {
                ...
                update(attributes: textAttributes, in: elementSpecificRange, from: element, updateType: updateType, removeBackgroundColor: false)
                recordElementExclusionRange(for: element, range: elementSpecificRange, in: &exclusionRanges)
            }
        }
        else {
            update(attributes: textAttributes, in: range, from: element, updateType: updateType, removeBackgroundColor: false)
            recordElementExclusionRange(for: element, range: range, in: &exclusionRanges)
        }
    }
    
    public func paintPseudoElements(element: Element, updateType: PaintingUpdateType = .add, exclusionRanges: inout [NSRange]) {
                    ...
                    if let pseudoRanges = pseudoRanges, pseudoRanges.count > 0 {
                        ...
                        if let ephemeralRanges = self.filterContext.ephemeralRanges(forElement: element, fromRanges: pseudoRanges) {
                            ...
                            for range in ephemeralRanges {
                                ...
                                update(attributes: textAttributes, in: range, from: element, updateType: updateType)
                                recordElementExclusionRange(for: element, range: range, in: &exclusionRanges)
                            }
                        }
                    }
                        ...
                        for range in pseudoRanges {
                            ...
                            if !exclusionRanges.isEmpty {
                                ...
                                for elementSpecificRange in elementSpecificRanges {
                                    ...
                                    remove(attributes: textAttributes, in: elementSpecificRange, from: element)
                                    recordElementExclusionRange(for: element, range: elementSpecificRange, in: &exclusionRanges)
                                }
                            }
                            else {
                                
                                remove(attributes: textAttributes, in: range, from: element)
                                recordElementExclusionRange(for: element, range: range, in: &exclusionRanges)
                            }
                        }
                    }
    ...
    func update(attributes: [NSAttributedString.Key : Any]?, in range: NSRange, from element: Element, updateType: PaintingUpdateType, removeBackgroundColor: Bool = true) {
            ...
            if renderingType != .complete {
                ...
                if !differentAttributesRanges.isEmpty {
                    ...
                    for range in differentAttributesRanges {
                        updateDifferentRange(attributes: attributes, in: range, from: element, updateType: updateType)
                    }
                }
                else {
                    #if (os(macOS) || os(iOS) || os(tvOS) || os(watchOS)) && DEBUG && DEBUG_LOGS_ENABLED
                ...
                let totalRange = self.totalRange(fromElement: element, range: range)
                updateDifferentRange(attributes: attributes, in: totalRange, from: element, updateType: updateType)
            }
        }
        else {
            #if (os(macOS) || os(iOS) || os(tvOS) || os(watchOS)) && DEBUG && DEBUG_LOGS_ENABLED
    ...
    func updateDifferentRange(attributes: [NSAttributedString.Key : Any], in range: NSRange, from element: Element, updateType: PaintingUpdateType) {
        ...
        case .add:
            ...
            #endif
            addAddedAttributesRange(AttributesRange(attributes, range, element.localName))
        ...
        case .set:
            ...
            #endif
            addSetAttributesRange(AttributesRange(attributes, range, element.localName))
        }
    ...
    func removeTextDecorationAttributes(attributes: [NSAttributedString.Key : Any]?, in range: NSRange, from element: Element) {
        ...
        if let attributes = attributes {
            ...
            for attribute in attributes {
                ...
                if contentString.containsAttribute(key, in:  range) {
                    ...
                    #endif
                    addDeleteAttributesRange(AttributesRange([attribute.key: attribute.value], range, element.localName))
                }
            }
        }
        else {
    ...
    private func eraseNode(node: Node) {
        
        if let element = node as? Element {
            
            var exclusionRanges = [NSRange]()
            
            erasePseudoElements(element: element, exclusionRanges: &exclusionRanges)
