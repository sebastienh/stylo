...
final class MarkdownConcurentRenderer: ConcurentRenderer, Visitor, HtmlRendererVisitor {
    ...
    let resourceComputedStyle: ComputedStyle
    ...
    private let renderedLock = ReadWriteLock()
    
    let contentStringLock = ReadWriteLock()
    
    init(resourceComputedStyle: ComputedStyle, renderingContext: RenderingContext,  document: HtmlDocument) {
    ...
    private func renderComplete(elements: ContiguousArray<Element>, deletedNodes: ContiguousArray<Node>?) {
        ...
        if let element = elements.first {
            ...
            self.remove(attributes: [StyloAttribute.headingTagBefore.key: true], in: completeRange, from: element, attributesOperations: &attributesOperations)
            
            self.addedAttributes.append(contentsOf: attributesOperations.addedAttributes)
            self.deletedAttributes.append(contentsOf: attributesOperations.deletedAttributes)
            self.setAttributes.append(contentsOf: attributesOperations.setAttributes)
        }
    ...
    private func renderFocusBloc(elements: ContiguousArray<Element>, deletedNodes: ContiguousArray<Node>?) {
        ...
        let nodeInfo = document.body.acceptSingle(self)
        
        if let attributesOperations = nodeInfo?.attributesOperations {
            self.deletedAttributes.append(contentsOf: attributesOperations.deletedAttributes)
            self.setAttributes.append(contentsOf: attributesOperations.setAttributes)
            self.addedAttributes.append(contentsOf: attributesOperations.addedAttributes)
        }
        
        let renderingResult = self.fastRendering(elements)
        
        return renderingResult.forEach { (attributesOperations) in
            self.deletedAttributes.append(contentsOf: attributesOperations.deletedAttributes)
            self.setAttributes.append(contentsOf: attributesOperations.setAttributes)
            self.addedAttributes.append(contentsOf: attributesOperations.addedAttributes)
        }
    ...
    private func slowRendering(_ elements: ContiguousArray<Element>) -> [AttributesOperations] {
        
        var elementsAttributesOperations: [Int: AttributesOperations] = [:]
        ...
        for (index, element) in elements.enumerated() {
            ...
            if !(htmlElment is HTMLBodyElement) && !(htmlElment is HTMLHtmlElement) {
                guard let nodeInfo = htmlElment.accept(self) else {
                    assertionFailure("Error: nodeInfo is nil")
                    continue
                }
                if let attributesOperations = nodeInfo.attributesOperations {
                    elementsAttributesOperations[index] = attributesOperations
                }
                addRendererTopElement(element)
            }
        }
        return elementsAttributesOperations.map { (arg) -> AttributesOperations in
            return arg.value
        }
    }
    
    private func fastRendering(_ elements: ContiguousArray<Element>) -> [AttributesOperations] {
        ...
        DispatchQueue.concurrentPerform(iterations: elements.count) { (index) in
            ...
            if !(htmlElment is HTMLBodyElement) && !(htmlElment is HTMLHtmlElement) {
                guard let nodeInfo = htmlElment.acceptRenderer(self) else {
                    assertionFailure("Error: nodeInfo is nil")
                    return
                }
                if let attributesOperations = nodeInfo.attributesOperations {
                    lock.withCriticalSection { [weak self] in
                        elementsAttributesOperations[index] = attributesOperations
                        self?.addRendererTopElement(element)
                    }
                }
            }
        }
        ...
        return lock.withCriticalSection {
            return elementsAttributesOperations.sorted { (first, second) -> Bool in
                return first.key < second.key
            }.map { (arg) -> AttributesOperations in
                return arg.value
            }
        }
    }
...
//
//            if let headerLevelValue = attributes[StyloAttribute.headingTagBefore.attributeKey] as? NSNumber {
//
...
//        #endif
    ...
    func visit(_ node: HTMLHtmlElement) -> RenderNodeInfo? {
        ...
        return RenderNodeInfo(node: nil, visitChildren: true, attributesOperations: nil)
    }
    ...
    func visit(_ node: HTMLBodyElement) -> RenderNodeInfo? {
        ...
        if renderingType == .complete {
            paintParagraphStyle(from: node, attributesOperation: &attributesOperations)
        }
        
        paintTextElement(element: node, attributesOperations: &attributesOperations)
        return RenderNodeInfo(node: nil, visitChildren: true, attributesOperations: attributesOperations)
    }
    ...
    private func paint(node: HTMLElement, withFocusOptions focusOptions: PseudoClassesOptions?) -> RenderNodeInfo? {
        ...
        if let headingElement = node as? HTMLHeadingElement {
            assignBeforeHeadingTagAttributes(to: headingElement, attributesOperation: &attributesOperations)
        }
        return RenderNodeInfo(node: node, visitChildren: true, attributesOperations: attributesOperations)
    }
    ...
    private func focusOptions(forElement node: HTMLElement) -> PseudoClassesOptions? {
        ...
        if let focusType = self.renderingContext.focusType {
            ...
            if let (options, range) = self.focusOptions(forElement: node, focusType: focusType) {
                
                switch focusType {
                case .bloc:
                    self.filterContext.updateFocusedRange(forElement: node, with: range)
                case .paragraph:
                    ...
                    if let parentFocusedRange = parentFocusedRange {
                        self.filterContext.updateFocusedRange(forElement: node, with: parentFocusedRange)
                    }
                    else {
                        self.filterContext.updateFocusedRange(forElement: node, with: range)
                    }
            ...
            else if let parentFocusedRange = parentFocusedRange, let range = node.range, parentFocusedRange.lowerBound <= range.lowerBound && range.upperBound <= parentFocusedRange.upperBound {
                ...
                case .paragraph: fallthrough
                case .sentence:
                    return self.filterContext.defaultPseudoClassesOptions(forElement: node)
                case .flash:
                    return nil
                }
            }
        ...
        case .bloc:
            ...
            if let isFirstResponder = self.isFirstResponder, isFirstResponder {
                ...
                if parentElementOptions.contains(.focus) {
                    ...
                    return contentStringLock.withReadLock {
    ...
    private func assignBeforeHeadingTagAttributes(to headingElement: HTMLHeadingElement, attributesOperation: inout AttributesOperations) {
        ...
        if attributeValue == nil {
            ...
            #if (os(macOS) || os(iOS) || os(tvOS) || os(watchOS)) && DEBUG && DEBUG_LOGS_ENABLED
            os_log("assignBeforeHeadingTagAttributes -> assign value: %@", log: Log.WriterCommon.all, type: .info, %%levelValue)
            os_log("assignBeforeHeadingTagAttributes -> range: %@", log: Log.WriterCommon.all, type: .info, %%totalRange)
            #endif
            
            updateDifferentRange(attributes: [StyloAttribute.headingTagBefore.key : levelValue], in: totalRange, from: headingElement, updateType: .add, attributesOperation: &attributesOperation)
        }
    ...
    private func paintParagraphStyle(from element: Element, attributesOperation: inout AttributesOperations) {
        ...
        if let paragraphStyle = paragraphStyle {
            ...
            if !paragraphStyle.isEmpty {
                ...
                self.update(attributes: paragraphStyle, in: self.contentString.completeRange, from: element, updateType: .set, attributesOperations: &attributesOperation)
                contentStringLock.withWriteLock {
