//
//  DynamicOrderedSet+SequenceType.swift
//  Common
//
...
import Foundation
extension DynamicOrderedSet: Sequence {
    
    // SequenceType protocol implementation
    public func makeIterator() -> DynamicOrderedGenerator<T> {
        
        return DynamicOrderedGenerator<T>(values: self.values.contents)
    }
}

// see http://lillylabs.no/2014/09/30/make-iterable-swift-collection-type-sequencetype/
public final class  DynamicOrderedGenerator<T> : IteratorProtocol {
